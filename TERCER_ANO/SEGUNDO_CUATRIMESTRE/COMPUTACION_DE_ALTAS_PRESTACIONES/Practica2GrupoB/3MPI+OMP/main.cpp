...
void rayTracingCPUP(Scene world, unsigned char* img, int w, int h, int ns = 10, int px = 0, int py = 0, int pw = -1, int ph = -1) {
    if (pw == -1) pw = w;
    if (ph == -1) ph = h;
    int patch_w = pw - px;
...
int main() {
    ...
    constexpr int frameSize = sizeof(unsigned char) * W * H * 3;
    ...
    if (rank == 0) {
        ...
        for (int i = 0; i < FRAMES; i++) {
            datas.at(i) = (unsigned char*)calloc(frameSize, 1);
        }
        ...
        while (true) {
            ...
            /* E1. Procesar la imagen */
                ...
                int patch_x_size = W;
                int patch_y_size = H / (NTHREADS);

                unsigned char* data = (unsigned char*)calloc(frameSize, 1);
                ...
                for (int i = 0; i < (NTHREADS); i++) {
                        int patch_x_start = 0;
                        int patch_x_end = patch_x_size;
                        int patch_y_start = i * patch_y_size;
                        int patch_y_end = (i + 1) * patch_y_size;

                        rayTracingCPUP(world, data, W, H, ns, patch_x_start, patch_y_start, patch_x_end, patch_y_end);
                }
    ...
    printf("LA EJECUCION HA DURADO %f\n", progend - progstart);
